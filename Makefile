...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "superfun", "tests")); system.time(test_check("superfun", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran superfun_$(VERSION).tar.gz
	rm -rf superfun.Rcheck/
